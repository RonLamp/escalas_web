...
import {Container, DivForm, ButtonForm, MultiSelectForm} from '../styles';
...
const CadProfGroup: React.FC = () => {
   ...
   const [profisssSelected, setProfisssSelected] = useState<IOptionsProps[]>(
      [],
   ); //Multi groups selecionadas
   const [profisssTSelected, setProfisssTSelected] = useState<IRelProfGroup[]>(
      [],
   ); //Multi groups selecionadas Totais
      ...
      if (idSelected === '0') {
         ...
         try {
            ...
            const response = await api.get(
               `/api/profsgroup/group/${idSelected}`,
            );
   ...
   const handleProfissChange = async (Profisss: IOptionsProps[]) => {
      ...
      const newProfisssTSelected: IRelProfGroup[] = Profisss.map(item => {
         const profissT = profisssTSelected.find(
            profissT => profissT.profiss_Id === item.value,
         );
         if (profissT) {
            return profissT;
         } else {
   ...
   return (
      <Container>
         <ContentHeader title="Profissionais por Escala" linecolor="#ca590d">
            <SelectInput
               options={dados}
